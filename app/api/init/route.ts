...
export async function POST(req: NextRequest) {
  try {
    ...
    const initSecret = body.secret || req.headers.get('x-init-secret');
    ...
    if (count > 0) {
      return NextResponse.json({ success: true, message: 'Packages already seeded', packageCount: count });
    }
    ...
    return NextResponse.json({ success: true, message: `Seeded ${packages.length} packages`, packageCount: packages.length });
  } catch (err: any) {
