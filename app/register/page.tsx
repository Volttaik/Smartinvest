...
export default function Register() {
  ...
  const [form, setForm] = useState({ username: "", email: "", password: "", confirmPassword: "", referralCode: "", profilePicture: "" });
  const router = useRouter();
  const auth = useAuth();

  const goNext = () => { setError(""); setDir(1); setStep(s => s + 1); };
            ...
            <AnimatePresence mode="wait" custom={dir}>
              {step === 0 && (
                <motion.div key="step0" custom={dir} variants={stepVariants} initial="enter" animate="center" exit="exit"
                  transition={{ duration: 0.32, ease: [0.22, 1, 0.36, 1] }} className="space-y-5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input type="email" placeholder="you@example.com" value={form.email}
                      onChange={e => setForm(f => ({ ...f, email: e.target.value }))} className="h-11 rounded-xl" />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input placeholder="Enter referral code if you have one" value={form.referralCode}
                      onChange={e => setForm(f => ({ ...f, referralCode: e.target.value.toUpperCase() }))} className="h-11 rounded-xl" />
                  </div>
                  ...
                  <AnimatePresence>
                    {form.password && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
                        exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.25 }} className="space-y-1.5 overflow-hidden">
                        {[["At least 6 characters", form.password.length >= 6],
                          ...
                          <div key={String(label)} className="flex items-center gap-2 text-xs">
                            <motion.div animate={{ backgroundColor: ok ? "#22c55e" : "#e5e7eb" }} transition={{ duration: 0.25 }}
                ...
                <motion.div key="step2" custom={dir} variants={stepVariants} initial="enter" animate="center" exit="exit"
                  ...
                  <div className="flex flex-col items-center gap-5">
                    <motion.button type="button" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                      transition={{ duration: 0.18 }} onClick={() => fileInputRef.current?.click()}
                    ...
                    {photoPreview && (
                      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                        ...
                        <button type="button" onClick={() => { setPhotoPreview(null); setForm(f => ({ ...f, profilePicture: "" })); }}
                          className="ml-auto text-xs text-muted-foreground hover:text-red-500 transition-colors">Remove</button>
                      </motion.div>
                    )}
                  </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <Label htmlFor="terms" className="text-sm text-muted-foreground leading-relaxed cursor-pointer">
                        I agree to the{" "}
                        <Link href="/terms" className="text-primary hover:underline" target="_blank">Terms & Conditions</Link> and{" "}
