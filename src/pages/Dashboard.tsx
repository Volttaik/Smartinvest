...
import {
  TrendingUp, TrendingDown, Bell, Settings, LogOut, BarChart3, Wallet,
  ArrowUpRight, ArrowDownRight, DollarSign, Activity, Menu, X, PieChart as PieIcon,
  CreditCard, Target, Layers, Zap, RefreshCw, Copy, Check, Package, AlertCircle,
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Link, useNavigate, useSearchParams } from "react-router-dom";
import { useAuth } from "@/lib/auth";
import { getDashboard, getPackages, investInPackage, fundWallet, withdraw, verifyPayment } from "@/lib/api";
...
export default function Dashboard() {
  ...
  const loadDashboard = useCallback(async () => {
    try {
      ...
      if (err.response?.status !== 401) showNotif("error", "Failed to load dashboard data.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleInvest = async (pkg: any) => {
    ...
    try {
      ...
      showNotif("error", err.response?.data?.error || "Investment failed.");
    } finally {
      setPkgLoading(false);
    }
  };
  ...
  const handleFund = async () => {
    if (!fundAmount || parseFloat(fundAmount) < 100) {
      showNotif("error", "Minimum deposit is ₦100");
      return;
    }
    setFundLoading(true);
    try {
          ...
          const handler = PaystackPop.setup({
            ...
            callback: async (response: any) => {
              try {
                ...
                showNotif("error", "Payment verification failed.");
              }
      ...
      showNotif("error", err.response?.data?.error || "Payment initialization failed.");
      setFundLoading(false);
    }
  };
  ...
  const handleWithdraw = async () => {
    ...
    try {
      await withdraw({ ...withdrawForm, amount: parseFloat(withdrawForm.amount) });
      showNotif("success", "Withdrawal initiated successfully!");
      setWithdrawForm({ amount: "", accountName: "", accountNumber: "", bankCode: "", bankName: "" });
      await loadDashboard();
      await refreshUser();
    } catch (err: any) {
      showNotif("error", err.response?.data?.error || "Withdrawal failed.");
    } finally {
      ...
      <aside className={`fixed lg:relative z-40 flex flex-col h-full bg-foreground text-primary-foreground w-64 shrink-0 transition-transform duration-300
        ...
        <div className="p-4 border-t border-white/10">
          <div className="flex items-center gap-3 mb-4 px-1">
            <div className={`w-9 h-9 rounded-full ${av.bg} flex items-center justify-center text-lg shrink-0`}>{av.icon}</div>
        ...
        <main className="flex-1 overflow-y-auto p-6">
          {loading ? (
            ...
            <AnimatePresence mode="wait">
                            ...
                            <div key={inv.id} className="p-4 rounded-xl bg-muted/30 border border-border">
                              <div className="flex items-center justify-between mb-2">
                                <div>
                                  ...
                                  <Badge className="ml-2 text-xs">{inv.tier}</Badge>
                                </div>
              ...
              {/* INVEST */}
              {activeTab === "invest" && (
                <motion.div key="invest" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
              ...
              {activeTab === "transactions" && (
                <motion.div key="transactions" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              {/* REFERRALS */}
              {activeTab === "referrals" && (
                <motion.div key="referrals" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
              ...
              {/* FUND WALLET */}
              {activeTab === "fund" && (
                <motion.div key="fund" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  ...
                  <div className="bg-card border border-border rounded-2xl p-6 space-y-5">
                    ...
                    <Button onClick={handleFund} disabled={fundLoading || !fundAmount}
                      className="w-full h-11 bg-primary text-white rounded-xl font-semibold hover:brightness-110">
                  ...
                  <div className="bg-card border border-border rounded-2xl p-6 space-y-5">
                    ...
                    {balance < 10000 && (
                      <div className="p-3 rounded-xl bg-yellow-50 border border-yellow-200 text-yellow-800 text-sm flex gap-2">
                        <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" />
                        You need at least ₦10,000 to withdraw. Current balance is insufficient.
                      </div>
                    )}
                    <div className="space-y-3">
                      {[
                    ...
                    <Button onClick={handleWithdraw} disabled={withdrawLoading || balance < 10000}
                      className="w-full h-11 bg-primary text-white rounded-xl font-semibold hover:brightness-110">
